...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`categories\` ADD COLUMN \`_status\` text DEFAULT 'draft';`)
  await db.run(sql`ALTER TABLE \`products\` ADD COLUMN \`_status\` text DEFAULT 'draft';`)

  // Add is_active column to categories
  await db.run(sql`ALTER TABLE \`categories\` ADD COLUMN \`is_active\` integer DEFAULT true;`)
  ...
  await db.run(sql`CREATE INDEX \`_categories_v_version_slug_idx\` ON \`_categories_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_categories_v_version__status_idx\` ON \`_categories_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_categories_v_version_updated_at_idx\` ON \`_categories_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_categories_v_version_created_at_idx\` ON \`_categories_v\` (\`version_created_at\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_products_v_version_slug_idx\` ON \`_products_v\` (\`version_slug\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version__status_idx\` ON \`_products_v\` (\`version__status\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_updated_at_idx\` ON \`_products_v\` (\`version_updated_at\`);`)
  await db.run(sql`CREATE INDEX \`_products_v_version_created_at_idx\` ON \`_products_v\` (\`version_created_at\`);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD COLUMN \`_categories_v_id\` integer REFERENCES \`_categories_v\`(\`id\`) ON DELETE cascade;`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD COLUMN \`_products_v_id\` integer REFERENCES \`_products_v\`(\`id\`) ON DELETE cascade;`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels__categories_v_id_idx\` ON \`payload_locked_documents_rels\` (\`_categories_v_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels__products_v_id_idx\` ON \`payload_locked_documents_rels\` (\`_products_v_id\`);`)
}
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` DROP COLUMN \`_categories_v_id\`;`)
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` DROP COLUMN \`_products_v_id\`;`)
  await db.run(sql`ALTER TABLE \`categories\` DROP COLUMN \`_status\`;`)
  await db.run(sql`ALTER TABLE \`categories\` DROP COLUMN \`is_active\`;`)
